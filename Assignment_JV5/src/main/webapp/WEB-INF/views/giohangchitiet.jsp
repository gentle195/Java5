...
<br>
<section>
    <form:form action="/gio-hang-chi-tiet/add-gio-hang-chi-tiet" method="post" onsubmit="return checkAdd()"
               modelAttribute="gioHangChiTiet">
        <div class="container text-center">
            <table class="table table-info table-striped" style="text-align: center">
                ...
                <tr>
                    <td colspan="6">
                        <button type="submit" class="btn btn-secondary">Thêm Giỏ Hàng Chi Tiết</button>
                    </td>
                </tr>
            </table>
        </div>
    ...
    <br>
    <hr>
    <h4 style="text-align: center;">Danh Sách Giỏ Hàng Chi Tiết</h4>
    <hr>
    <table class="table table-info table-striped" style="text-align: center">
        <tr>
            <th>Mã Giỏ Hàng</th>
            ...
            <th>Số Lượng</th>
            <th>Đơn Giá</th>
            <th>Sau Khi Giảm Giá</th>
            <th>Action</th>
        </tr>
        <c:forEach items="${list}" var="gioHangChiTiet">
            <tr>
                <td>${gioHangChiTiet.id.gioHang.ma}</td>
                <td>${gioHangChiTiet.id.chiTietSanPham.sanPham.ten}</td>
                <td>${gioHangChiTiet.soLuong}</td>
                <td>${gioHangChiTiet.donGia}</td>
                <td>${gioHangChiTiet.donGiaKhiGiam}</td>
                <td>
                    <a href="/gio-hang-chi-tiet/detail-gio-hang-chi-tiet/${gioHangChiTiet.id.gioHang.id}/${gioHangChiTiet.id.chiTietSanPham.id}"
                       class="btn btn-success "
                       tabindex="-1"
                       role="button"
                       onclick="return checkDetail()">Detail</a>
                    <a href="/gio-hang-chi-tiet/view-update-gio-hang-chi-tiet/${gioHangChiTiet.id.gioHang.id}/${gioHangChiTiet.id.chiTietSanPham.id}"
                       class="btn btn-success "
                       tabindex="-1"
                       role="button"
                       aria-disabled="true" onclick="return checkViewUpdate()">Update</a>
                    <a href="/gio-hang-chi-tiet/remove-gio-hang-chi-tiet/${gioHangChiTiet.id.gioHang.id}/${gioHangChiTiet.id.chiTietSanPham.id}"
                       class="btn btn-success "
                       tabindex="-1"
                       role="button"
    ...
    <nav aria-label="Page navigation example">
        <ul class="pagination justify-content-center pagination-lg">
            ...
            <c:forEach begin="1" end="${total}" varStatus="status">
                <li class="page-item">
                    <a href="/gio-hang-chi-tiet/hien-thi?pageNum=${status.index -1}"
                       class="page-link">${status.index}</a>
                </li>
            </c:forEach>
            <li class="page-item"><a class="page-link" href="/gio-hang-chi-tiet/hien-thi?pageNum=${total-1}">Last</a>
            </li>
        </ul>
    </nav>
    </div>
